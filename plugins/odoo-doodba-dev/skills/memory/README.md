...
> Persistent Odoo project memory for tracking models, fields, views, and user context across agent sessions

## Overview

The **Odoo Development Memory** skill solves a critical problem in AI-assisted Odoo development: **context loss across agent sessions and subagent transitions**. When working with Claude Code on Odoo projects, especially with complex tasks that involve multiple subagents, important decisions about models, fields, views, security, user requirements, and Q&A context often get lost. This skill provides a persistent memory system specifically designed for Odoo development to maintain continuity.

## The Problem

Without persistent memory in Odoo development:

1. **Subagents start fresh** - Each subagent launch loses previous model/field/view context
2. **Session interruptions** - Resuming work requires re-explaining model relationships and user requirements
3. **Repeated questions** - User must answer the same questions about business logic and constraints
4. **Lost decisions** - Model design, field configurations, and view layouts decided earlier are forgotten
5. **Missing Q&A context** - Valuable user clarifications and analysis results are lost
6. **Context duplication** - Wasted time re-establishing Odoo-specific context

## The Solution

With Odoo Development Memory:

1. **Odoo-specific storage** - Track models, fields, views, security, workflows, and user interactions
2. **Prioritized context** - User Q&A and analysis results shown first to preserve valuable insights
3. **Quick retrieval** - Fast search and retrieval of Odoo-specific information
4. **Structured organization** - Odoo-specific categories (model_design, field_config, view_design, etc.)
5. **Project-scoped** - Each Odoo project has its own isolated memory
6. **Export/import** - Share Odoo development context between team members
...
```bash
# ALWAYS START HERE: Get an Odoo context summary
python -m scripts.get_context

# This shows you organized overview of (prioritized order):
# - User Interactions & Q&A - Critical user clarifications and analysis
# - Important Findings - Discoveries about existing Odoo code
# - Requirements - User functional specifications
# - Current Context - Active development tasks
# - Model Design Decisions - Odoo model architecture
# - Field Configuration Decisions - Field types and relationships
# - View & UI Design - Forms, trees, kanban, qweb templates
# - Security Configurations - Access rights and record rules
# - And more...

# Store a model design decision
python -m scripts.store "model.sale_order.extend" \
  "Extended sale.order with custom_delivery_instructions Text field" \
  --category model_design \
  --context "User requested per-order delivery notes visible to warehouse" \
  --tags "model,sale,inheritance,text"

# Store user interaction/clarification
python -m scripts.store "interaction.user_q1.delivery_notes" \
  "Q: Should delivery notes be editable after confirmation? A: No, lock after confirm" \
  --category user_interaction \
  --tags "qa,requirement,delivery,security"

# Retrieve it later
python -m scripts.retrieve "model.sale_order.extend"

# Search for Odoo-specific information
...
**Always use this first** to get a coherent, organized overview of the Odoo project state:

```bash
# Get formatted Odoo context summary (user Q&A prioritized first)
python -m scripts.get_context
...
# Filter by Odoo-specific category
python -m scripts.get_context --category model_design
python -m scripts.get_context --category user_interaction
